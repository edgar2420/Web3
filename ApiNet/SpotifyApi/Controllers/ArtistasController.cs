    ...
    public class ArtistasController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ArtistaDto>>> GetArtista()
        {
            try
            {
                ...
                Console.WriteLine("Número de artistas obtenidos: " + artistas.Count); // Log de depuración
                return Ok(artistas);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ArtistaDto>> GetArtista(int id)
        {
            ...
            var artistaDto = new ArtistaDto
            {
                ...
                GeneroId = artista.Genero?.GeneroId ?? 0, // Asegura que si no hay género, sea 0
                Url = $"{Request.Scheme}://{Request.Host}/artistas/{artista.ArtistaId}.jpg" // URL dinámica
            };
